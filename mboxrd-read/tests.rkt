...
(module+ test
  ...
  (define example-maildir (build-path here "example-maildir"))

  (define test-stream (maildir-parse example-maildir))
  ...
  (check-equal? (stream-length (maildir-parse (build-path example-maildir
                                                          ".sub-maildir")))
                2)

  (check-equal? (maildir-num-unread example-maildir) 2))
